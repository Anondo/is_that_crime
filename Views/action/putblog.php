...
if(!empty($_POST["title"]) && !empty($_POST["location"]) && !empty($_POST["category"]) && !empty($_POST["body"]))
{
    ...
    if($success)
         header("Location: http://localhost:<?php echo  {$_SERVER["SERVER_PORT"]};?>/Projects/aiub project/index.php");
    else
        echo "<a href = 'http://localhost:<?php echo  {$_SERVER["SERVER_PORT"]};?>/Projects/aiub project/Views/crime_post.php'>Something Went Wrong</a>";

}
else
{
    echo "<a href = 'http://localhost:<?php echo  {$_SERVER["SERVER_PORT"]};?>/Projects/aiub project/Views/crime_post.php'>Title & Description required</a>";
}
