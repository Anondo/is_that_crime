...
if(isset($_POST["username_email"]) && isset($_POST["password"]))
{
    ...
    if($login->isUserValid($username_or_email , $password , "username") ||$login->isUserValid($username_or_email , $password , "email"))
    {
        ...
        $login->log_in($row["username"] , $row["user_id"] , " http://localhost:<?php echo  {$_SERVER["SERVER_PORT"]};?>/Projects/aiub%20project/index.php");
    }
    else
    {
        echo "<h1><a href = 'http://localhost:<?php echo  {$_SERVER["SERVER_PORT"]};?>/Projects/aiub%20project/Views/login.php'>Invalid User</a></h1>";
    }
}
